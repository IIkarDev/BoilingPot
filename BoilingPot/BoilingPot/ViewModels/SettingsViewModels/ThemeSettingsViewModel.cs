...
// Для AvaloniaXamlLoader.Load
// Для ThemeVariant, IStyle, ResourceDictionary, Styles
// Базовый класс RxUI
// Для [Reactive]
// Для Debug
// Для Linq
...
// Для WhenAnyValue, SelectMany, Subscribe
...
public class ThemeSettingsViewModel : ViewModelBase
{
    ...
    public ThemeSettingsViewModel()
    {
        ...
        AccentPalettes = new List<AccentPaletteInfo>
        {
            ...
            new("Taup (По ум.)", "avares://BoilingPot/Resources/Palettes/DefaultAccentPalette.axaml"),
            new("Purple", "avares://BoilingPot/Resources/Palettes/PurpleAccentPalette.axaml"),
            new("Red", "avares://BoilingPot/Resources/Palettes/RedAccentPalette.axaml"),
            new("Blue", "avares://BoilingPot/Resources/Palettes/BlueAccentPalette.axaml"),
            new("Teal", "avares://BoilingPot/Resources/Palettes/TealAccentPalette.axaml"),
            new("Orange", "avares://BoilingPot/Resources/Palettes/OrangeAccentPalette.axaml"),
            new("Gray", "avares://BoilingPot/Resources/Palettes/GrayAccentPalette.axaml"),
            new("Green", "avares://BoilingPot/Resources/Palettes/GreenAccentPalette.axaml")
        };

        // Устанавливаем начальную выбранную палитру (первую в списке)
        SelectedAccentPalette = AccentPalettes.FirstOrDefault();
        Debug.WriteLine(
            $"[ThemeSettingsVM] Конструктор RxUI: Выбрана начальная палитра: {SelectedAccentPalette?.Name ?? "null"}");
    ...
    private void ApplyTheme(string themeKey)
    {
        ...
        var newVariant = themeKey switch
        {
        ...
        {
            Debug.WriteLine(
                "!!! ThemeSettingsVM: ApplyTheme: Application.Current равен null. Не удалось установить тему.");
        }
    }
    ...
    private void ApplyAccentPalette(AccentPaletteInfo? paletteInfo)
    {
        ...
        {
            Debug.WriteLine(
                "[ThemeSettingsVM] ApplyAccentPalette: paletteInfo или Application.Current равен null. Палитра не применена.");
            return;
        }

        Debug.WriteLine(
            $"[ThemeSettingsVM] ApplyAccentPalette: Применение палитры '{paletteInfo.Name}' из '{paletteInfo.ResourceUri}'");
        ...
        try
        {
            ...
            foreach (var key in accentKeys)
                ...
                else
                {
                    ...
                    Debug.WriteLine(
                        $"[ThemeSettingsVM] ApplyAccentPalette: ПРЕДУПРЕЖДЕНИЕ: Ресурс '{key}' не найден в палитре '{paletteInfo.Name}'");
                }

            if (appliedCount > 0)
                Debug.WriteLine(
                    $"[ThemeSettingsVM] ApplyAccentPalette: Успешно применена палитра: {paletteInfo.Name}. Обновлено {appliedCount} ресурсов.");
            else
                Debug.WriteLine(
                    $"!!! ThemeSettingsVM: ApplyAccentPalette: Не удалось применить палитру {paletteInfo.Name}. Ни один ожидаемый ресурс не найден.");
        }
        catch (Exception ex)
        {
            // Обработка ошибок (например, файл палитры не найден или содержит ошибки XAML)
            Debug.WriteLine(
                $"!!! ThemeSettingsVM: ApplyAccentPalette: Ошибка при применении палитры '{paletteInfo?.Name}': {ex}");
        }
    }

    // --- Вспомогательный класс для хранения информации о палитре ---
    // Это не ViewModel, а простая модель данных для ComboBox.
    public class AccentPaletteInfo(string name, string resourceUri)
    {
        public string Name { get; } = name; // Имя для отображения в ComboBox
        public string ResourceUri { get; } = resourceUri; // URI файла палитры (.axaml)
